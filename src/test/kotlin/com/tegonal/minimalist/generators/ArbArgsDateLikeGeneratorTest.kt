...
class ArbArgsDateLikeGeneratorTest : AbstractArbArgsGeneratorTest<Any>() {
	...
	override fun createGenerators() = ZonedDateTime.now().let { nowZonedDateTime ->
		...
		sequenceOf(
			Tuple(
				...
				modifiedArb.zonedDateTimeFromUntil(nowZonedDateTime, nowZonedDateTime.plusHours(3), ChronoUnit.MINUTES),
				(0L until 3 * 60).map { nowZonedDateTime.plusMinutes(it) }
			),
			Tuple(
				"localDateTimeFromUntil",
				modifiedArb.localDateTimeFromUntil(nowLocalDateTime, nowLocalDateTime.plusDays(1), ChronoUnit.HOURS),
