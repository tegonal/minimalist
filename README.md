...
methods like `filter` require that it gets materialised first. Such methods are signified with a `Materialised` suffix.
...
The cleanest way to define that two `ArgsGenerators` shall be combined, is to use `Tuple` (from ch.tutteli.kbox)
which exists up to `Tuple9`:
...
class CombineTupleTest : PredefinedArgsSourceProviders {
...
```kotlin
class CombineManuallyTest : PredefinedArgsSourceProviders {
	...
	companion object {
		...
		fun numbersAndChar() = run { // use run to let the compiler infer the return type
			val numbers = Tuple(
				arb.int().combine(arb.long()), // combines them into a ArbArgsGenerators<Tuple2<Int, Long>>
				arb.double(),
				arb.bigIntFromUntil(BigInt.ZERO, BigInt.TEN)
...
`Pair`/`Triple`, i.e. not flattened, then define e.g. a `data class`.
...
```kotlin
class CombineDependentTest : PredefinedArgsSourceProviders {
...
Filtering an `ArgsGenerator` can be done via `filter`
